...
public class MusicDataController : MonoBehaviour
{
    ...
    private const float TIME_THRESHOLD = 0.01f; // e.g. 50 milliseconds
    ...
    void TriggerBeatAnimation()
    {
        ...
        for(int i =0; i < beatObject.transform.childCount; i++)
        {
            beatObject.transform.GetChild(i).gameObject.GetComponent<ParticleSystem>().Play();
        }
    ...
    void ResetMaterial()
    {
        // Change the material back to default//idf this is a problem use for loop fpr children
        for (int i = 0; i < beatObject.transform.childCount; i++)
        {
            beatObject.transform.GetChild(i).gameObject.GetComponent<ParticleSystem>().Stop();
        }
    ...
    {
        for(int i =0; i < energyObject.transform.childCount; i++)
        {
            GameObject curr = energyObject.transform.GetChild(i).gameObject;
            // Only transition if the current state is not already high energy
            if (curr.GetComponent<Renderer>().material != highEnergyMaterial)
            {
                curr.GetComponent<Renderer>().material = highEnergyMaterial;
    ...
    void TriggerLowEnergyState()
    {
        for (int i = 0; i < energyObject.transform.childCount; i++)
        {
            ...
            if (curr.GetComponent<Renderer>().material != lowEnergyMaterial)
            {
                curr.GetComponent<Renderer>().material = lowEnergyMaterial;
                //highEnergyLight.enabled = true;
                //lowEnergyLight.enabled = false;
            }
        }
    ...
    void UpdateFrequencyData(int index)
    {
        ...
        bassObject.GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(0, bassValue * yAnimationScale + yAnimationScaleBias);
        midObject.GetComponent<SkinnedMeshRenderer>().SetBlendShapeWeight(0, midValue * yAnimationScale + yAnimationScaleBias);
